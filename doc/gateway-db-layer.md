...
当前已经落了通用插件 KV Store 和积分系统。操作日志、媒体队列、群聊记录等能力会在这个基础上继续扩展。
...
5. 插件在 `handle(context)` 里通过 `context.services.pluginData` 和 `context.services.points` 使用它们。
...
PostgreSQL 用于长期事实数据，例如插件状态、积分流水、操作日志、媒体链接、文本记录等。
